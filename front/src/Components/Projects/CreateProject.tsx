...
function CreateProject({ visible, setModalMessage, setModalColor }: { visible: Function, setModalMessage: Function, setModalColor: Function }) {
    ...
    return (
        <div className='fixed w-9/12 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform z-10'>
            <div className='w-full bg-white font-bold m-auto rounded-xl shadow-2xl min-h-fit p-2 border-2 border-black'>
                ...
                <form onSubmit={(e) => handleCreate(e)} className='flex flex-col p-2 gap-2'>
                    ...
                    <input onChange={(e) => setProjectName(e.target.value)} value={projectName} className='transform transition ease-in-out 150 border-2 border-black pr-2 pl-2 p-1 rounded-xl focus:outline-none focus:border-green-500' type="text" name="issueName" />
